...
//				var_dump($html);die();
		...
		preg_match_all("#class=\"v\">([\s\S]+?)<div class=\"v-meta-overlay#",$html,$hlist);
		foreach($hlist[1] as $key=>$val)
		{
			$l = array();
			preg_match("#src=\"([^\"]+?)\" replace=\"[^\"]+?\"   alt=\"([^\"]+?)\"#",$val,$p1);
			preg_match("#class=\"v-time\">([^<>]+?)</#",$val,$p2);
			preg_match("#class=\"v-num\">([^<>]+?)</#",$val,$p3);
			preg_match("#href=\"http://v.youku.com/v_show/id_([^\"]+?)\.html#",$val,$p4);
			preg_match("#v-upload-date\"  >([^<>]+?)</#",$val,$p5);
